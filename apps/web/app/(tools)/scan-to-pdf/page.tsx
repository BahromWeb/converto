...
import { Upload, Download, ScanLine, ArrowRight } from "lucide-react";
...
export const metadata: Metadata = {
  title: "Scan to PDF Online — Photos of Paper → One Searchable PDF, Free",
  description: "Photograph paper documents with your phone, get back one searchable PDF. Free, no sign-up, no watermark.",
  alternates: {
  ...
  openGraph: {
    title: "Scan to PDF Online — Photos of Paper → One Searchable PDF, Free",
    description: "Photograph paper documents with your phone, get back one searchable PDF. Free, no sign-up, no watermark.",
    url: "https://convertpdfgo.com/scan-to-pdf",
    images: [{ url: "/og.png", width: 1200, height: 630, alt: "convertpdfgo — free PDF tools", type: "image/png" }],
  },
};

const howToSteps = [
  { icon: Upload, name: "Upload your photos", text: "Drop multiple JPG/PNG/HEIC photos. Each becomes a page in the output PDF." },
  { icon: ScanLine, name: "We bundle + OCR", text: "Photos become pages in a single PDF, then OCR adds a searchable text layer underneath." },
  { icon: Download, name: "Download the scan", text: "One searchable PDF, in the same order as the photos. Send it like a normal scan." },
];

const faqs = [
  { q: "Is the OCR accurate on phone photos?", a: "Yes — modern phone cameras shoot at high enough resolution that OCR works well. Lighting matters more than camera quality; even illumination beats high megapixels." },
  { q: "Can I re-order the photos?", a: "Yes — drag photo thumbnails to re-order before submission. The PDF preserves your order." },
  { q: "Are my files private?", a: "Files are encrypted in transit (TLS 1.3) and at rest, then auto-deleted within one hour." },
];

const relatedTools = [
  { slug: "ocr", name: "OCR PDF", hint: "OCR an existing PDF instead" },
  { slug: "jpg-to-pdf", name: "JPG to PDF", hint: "Just bundle, no OCR" },
  { slug: "compress", name: "Compress PDF", hint: "Shrink the output" },
];
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        headline: "How to Turn Phone Photos of Paper into a Searchable PDF — Free",
        description: "Photograph paper documents with your phone, get back one searchable PDF. Free, no sign-up, no watermark.",
        ...
        mainEntityOfPage: { "@type": "WebPage", "@id": "https://convertpdfgo.com/scan-to-pdf" },
        speakable: { "@type": "SpeakableSpecification", cssSelector: ["h1", ".speakable-faq"] },
        inLanguage: "en", about: { "@type": "Thing", name: "Scan to PDF" },
        keywords: "scan to pdf, phone scanner pdf, photos to searchable pdf",
      }) }} />

      <ToolPageShell tool={tool} index="25" variant="photos → one PDF">
        <ScanToPdfClient />
        ...
        <div className="mt-8 sm:mt-12 grid gap-6 lg:gap-8 lg:grid-cols-[1fr_1fr]">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Scan to PDF FAQ</h2>
