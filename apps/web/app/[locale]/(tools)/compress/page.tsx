...
import { Upload, Minimize2, Download, ArrowRight } from "lucide-react";
...
const baseMetadata: Metadata = {
  title: "Compress PDF Online — Shrink PDF Size 50-90%, Free, No Watermark",
  description:
    "Compress PDF online — keep text searchable, drop file size 50-90%. Free, no sign-up, no watermark. Works on mobile and desktop.",
  alternates: {
    languages: {
  ...
  openGraph: {
    title: "Compress PDF Online — Shrink PDF Size 50-90%, Free, No Watermark",
    description: "Shrink any PDF by 50-90% — keep text searchable. Free, no sign-up, no watermark.",
    url: "https://convertpdfgo.com/compress",
...
export async function generateMetadata(
  ...
  return localizeToolMetadata(baseMetadata, "compress", locale);
}

const howToSteps = [
  { icon: Upload,    name: "Upload your PDF",            text: "Drop the PDF or click to pick it from your device." },
  { icon: Minimize2, name: "Pick a compression level",    text: "Low, medium, or high — each one trades a bit of image quality for more shrinkage." },
  { icon: Download,  name: "Download the smaller PDF",    text: "Open it in any reader — same content, much smaller file." },
];

const faqs = [
  { q: "How much smaller will the PDF be?", a: "Image-heavy PDFs shrink 60-90% on medium. Text-only PDFs shrink less — sometimes 20% — because there's not much to compress." },
  { q: "Will the text stay searchable?", a: "Yes. We don't rasterise the text — searches, copy-paste, OCR layers all keep working on the compressed output." },
  { q: "Are my files private?", a: "Files are encrypted in transit (TLS 1.3) and at rest, then auto-deleted within one hour." },
];

const relatedTools = [
  { slug: "merge",      name: "Merge PDF",   hint: "Combine the smaller PDFs" },
  { slug: "split",      name: "Split PDF",   hint: "Break it up first if needed" },
  { slug: "pdf-to-jpg", name: "PDF to JPG",  hint: "Export pages as images instead" },
];
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        description: "Shrink any PDF by 50-90% — keep text searchable. No software, no sign-up, no watermark.",
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        headline: "How to Compress a PDF Online — Free and Watermark-Free",
        description: "Shrink any PDF by 50-90% with convertpdfgo — text stays searchable, no quality loss on documents. Free, no sign-up.",
        ...
        mainEntityOfPage: { "@type": "WebPage", "@id": "https://convertpdfgo.com/compress" },
        speakable: { "@type": "SpeakableSpecification", cssSelector: ["h1", ".speakable-faq"] },
        inLanguage: "en", about: { "@type": "Thing", name: "PDF compression" },
        keywords: "compress pdf, shrink pdf, reduce pdf size, make pdf smaller, compress pdf online free",
      })}} />

      <ToolPageShell tool={tool} locale={locale} index="11" variant="low · medium · high">
        <CompressCard />
        ...
        <div className="mt-8 sm:mt-12 grid gap-6 lg:gap-8 lg:grid-cols-[1fr_1fr]">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Compress PDF FAQ</h2>
