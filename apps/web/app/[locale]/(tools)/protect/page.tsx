...
import { Upload, Lock, Download, ArrowRight } from "lucide-react";
...
const baseMetadata: Metadata = {
  title: "Protect PDF Online — Add Password to PDF Free, No Watermark",
  description:
    "Add a password to any PDF online — AES-128 encryption, no sign-up, no watermark. Free, works on mobile and desktop.",
  alternates: {
    languages: {
  ...
  openGraph: {
    title: "Protect PDF Online — Add Password to PDF Free, No Watermark",
    description: "Lock any PDF with a password — AES-128 encryption. Free, no sign-up, no watermark.",
    url: "https://convertpdfgo.com/protect",
...
export async function generateMetadata(
  ...
  return localizeToolMetadata(baseMetadata, "protect", locale);
}

const howToSteps = [
  { icon: Upload,   name: "Upload your PDF",     text: "Drop the PDF or click to pick it from your device." },
  { icon: Lock,     name: "Set a password",       text: "Type a password the recipient needs to open the file. Pick something memorable." },
  { icon: Download, name: "Download the locked PDF", text: "Send the password through a separate channel — text, Signal, in-person — never in the same email." },
];

const faqs = [
  {
    q: "What encryption does it use?",
    a: "AES-128 (the same algorithm used by Adobe Acrobat's default password mode). Strong enough for everyday business documents and HR paperwork.",
  },
  {
    q: "Can I remove the password later?",
    a: "Yes — use our Unlock PDF tool with the same password. We don't crack passwords (the math doesn't allow it), but if you know it, removing it is one click.",
  },
  {
...
const relatedTools = [
  { slug: "unlock",    name: "Unlock PDF",   hint: "Remove a password you know" },
  { slug: "sign",      name: "Sign PDF",     hint: "Lock it after signing" },
  { slug: "watermark", name: "Watermark",    hint: "Stamp 'CONFIDENTIAL' on every page" },
];
  ...
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        name: "How to add a password to a PDF online",
        description: "Lock any PDF with an AES-128 password — no software, no sign-up, no watermark.",
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        headline: "How to Protect a PDF with a Password — Free and Watermark-Free",
        description: "Lock any PDF with AES-128 password protection with convertpdfgo — free, no sign-up, no watermark.",
        ...
        mainEntityOfPage: { "@type": "WebPage", "@id": "https://convertpdfgo.com/protect" },
        speakable: { "@type": "SpeakableSpecification", cssSelector: ["h1", ".speakable-faq"] },
        inLanguage: "en", about: { "@type": "Thing", name: "PDF password protection" },
        keywords: "protect pdf, password protect pdf, add password to pdf, encrypt pdf, lock pdf online free",
      })}} />

      <ToolPageShell tool={tool} locale={locale} index="09" variant="aes-128">
        <ProtectCard />
        ...
        <div className="mt-8 sm:mt-12 grid gap-6 lg:gap-8 lg:grid-cols-[1fr_1fr]">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Protect PDF FAQ</h2>
