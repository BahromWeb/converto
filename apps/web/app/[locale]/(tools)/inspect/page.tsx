...
import { Upload, Download, SearchCheck, ArrowRight } from "lucide-react";
...
const baseMetadata: Metadata = {
  title: "Inspect PDF Online — Page Count, Metadata, Encryption, Free",
  description: "Get instant PDF info — page count, title, author, keywords, encryption status, file size. Free, no sign-up.",
  alternates: {
    languages: {
  ...
  openGraph: {
    title: "Inspect PDF Online — Page Count, Metadata, Encryption, Free",
    description: "Get instant PDF info — page count, title, author, keywords, encryption status, file size. Free, no sign-up.",
    url: "https://convertpdfgo.com/inspect",
...
export async function generateMetadata(
  ...
  return localizeToolMetadata(baseMetadata, "inspect", locale);
}

const howToSteps = [
  { icon: Upload, name: "Upload your PDF", text: "Drop the PDF or click to pick it from your device." },
  { icon: SearchCheck, name: "We read the header", text: "No conversion, no recompression — we just parse the metadata block." },
  { icon: Download, name: "View results", text: "Title, author, keywords, encryption status, PDF version — all in under a second." },
];

const faqs = [
  { q: "Does it modify the PDF?", a: "No — inspect is read-only. The original file is not changed in any way." },
  { q: "Can I check encryption status without unlocking?", a: "Yes — inspect surfaces encryption status without needing the password." },
  { q: "Are my files private?", a: "Files are encrypted in transit (TLS 1.3) and at rest, then auto-deleted within one hour." },
];

const relatedTools = [
  { slug: "metadata", name: "Edit Metadata", hint: "Change what inspect shows" },
  { slug: "detect-blank", name: "Detect Blank Pages", hint: "Spot empty pages" },
  { slug: "compress", name: "Compress PDF", hint: "Shrink based on what inspect shows" },
];
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        headline: "How to Check PDF Metadata, Page Count, Encryption Online — Free",
        description: "Get instant PDF info — page count, title, author, keywords, encryption status, file size. Free, no sign-up.",
        ...
        mainEntityOfPage: { "@type": "WebPage", "@id": "https://convertpdfgo.com/inspect" },
        speakable: { "@type": "SpeakableSpecification", cssSelector: ["h1", ".speakable-faq"] },
        inLanguage: "en", about: { "@type": "Thing", name: "Inspect PDF" },
        keywords: "inspect pdf, pdf metadata, pdf page count",
      }) }} />

      <ToolPageShell tool={tool} locale={locale} index="19" variant="instant metadata">
        <InspectClient />
        ...
        <div className="mt-8 sm:mt-12 grid gap-6 lg:gap-8 lg:grid-cols-[1fr_1fr]">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Inspect PDF FAQ</h2>
