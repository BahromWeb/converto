...
/** OCR with layout/structure info — useful for downstream parsing. */
export function OcrStructuredClient() {
  const [lang, setLang] = useState("eng");
  return (
    <SimpleToolClient
      op="ocr-structured"
      cta="Run structured OCR →"
      resultFilename={(f) => (f?.name ?? "result").replace(/\.pdf$/i, "") + ".hocr"}
      downloadLabel="Download HOCR"
      buildExtraBody={() => ({ lang })}
      extraFields={
