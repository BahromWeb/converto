...
import { Upload, Download, Languages, ArrowRight } from "lucide-react";
...
const baseMetadata: Metadata = {
  title: "Detect PDF Language Online — Free, 100+ Languages Recognised",
  description: "Find out what language a PDF is in — Tesseract will tell you. Free, no sign-up.",
  alternates: {
  ...
  openGraph: {
    title: "Detect PDF Language Online — Free, 100+ Languages Recognised",
    description: "Find out what language a PDF is in — Tesseract will tell you. Free, no sign-up.",
    url: "https://convertpdfgo.com/ocr-detect-lang",
...
export async function generateMetadata(
  ...
  return localizeToolMetadata(baseMetadata, "ocr-detect-lang", locale);
}

const howToSteps = [
  { icon: Upload, name: "Upload your PDF", text: "Drop the PDF or click to pick it from your device. Image-based PDFs work; born-digital PDFs work too." },
  { icon: Languages, name: "We scan a sample page", text: "Tesseract identifies the script and language from a representative page sample." },
  { icon: Download, name: "Get the language code", text: "ISO codes (eng, rus, uzb, etc.) so you can run the right OCR / Extract Text afterward." },
];

const faqs = [
  { q: "Does it work on mixed-language documents?", a: "Yes — it returns the dominant language. For balanced bilingual docs (Uzbek + Russian, say), expect the more text-heavy script." },
  { q: "How accurate?", a: "Very high on clean scans — Tesseract distinguishes between similar scripts (Cyrillic Russian vs Cyrillic Uzbek) reliably." },
  { q: "Are my files private?", a: "Files are encrypted in transit (TLS 1.3) and at rest, then auto-deleted within one hour." },
];

const relatedTools = [
  { slug: "ocr", name: "OCR PDF", hint: "Run OCR once you know the language" },
  { slug: "ocr-extract-text", name: "OCR Extract Text", hint: "Direct text extraction" },
  { slug: "inspect", name: "Inspect PDF", hint: "Get other metadata" },
];
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        headline: "How to Detect a PDF's Language Online — Free, 100+ Languages",
        description: "Find out what language a PDF is in — Tesseract will tell you. Free, no sign-up.",
        ...
        mainEntityOfPage: { "@type": "WebPage", "@id": "https://convertpdfgo.com/ocr-detect-lang" },
        speakable: { "@type": "SpeakableSpecification", cssSelector: ["h1", ".speakable-faq"] },
        inLanguage: "en", about: { "@type": "Thing", name: "Detect Language" },
        keywords: "detect pdf language, pdf language detector, ocr detect language",
      }) }} />

      <ToolPageShell tool={tool} locale={locale} index="28" variant="100+ languages">
        <OcrDetectLangClient />
        ...
        <div className="mt-8 sm:mt-12 grid gap-6 lg:gap-8 lg:grid-cols-[1fr_1fr]">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Detect Language FAQ</h2>
