...
import { Upload, Download, Tags, ArrowRight } from "lucide-react";
...
const baseMetadata: Metadata = {
  title: "Edit PDF Metadata Online — Title, Author, Keywords, Free",
  description: "Rewrite a PDF's title, author, subject, keywords, creator. Free, no sign-up, no watermark.",
  alternates: {
    languages: {
  ...
  openGraph: {
    title: "Edit PDF Metadata Online — Title, Author, Keywords, Free",
    description: "Rewrite a PDF's title, author, subject, keywords, creator. Free, no sign-up, no watermark.",
    url: "https://convertpdfgo.com/metadata",
...
export async function generateMetadata(
  ...
  return localizeToolMetadata(baseMetadata, "metadata", locale);
}

const howToSteps = [
  { icon: Upload, name: "Upload your PDF", text: "Drop the PDF or click to pick it from your device." },
  { icon: Tags, name: "Edit metadata fields", text: "Title, author, subject, keywords, creator — fill what you want to change. Leave others blank to keep them." },
  { icon: Download, name: "Download the updated PDF", text: "Same content, fresh metadata — useful for search engines, content systems, libraries." },
];

const faqs = [
  { q: "Why does PDF metadata matter?", a: "Search engines index it. Content management systems sort by it. Librarians catalogue it. Junk metadata = junk results." },
  { q: "Can I remove metadata entirely for privacy?", a: "Yes — leave fields blank and submit. The output PDF will have empty metadata for those fields." },
  { q: "Are my files private?", a: "Files are encrypted in transit (TLS 1.3) and at rest, then auto-deleted within one hour." },
];

const relatedTools = [
  { slug: "inspect", name: "Inspect PDF", hint: "See current metadata first" },
  { slug: "protect", name: "Protect PDF", hint: "Add a password too" },
  { slug: "merge", name: "Merge PDF", hint: "Combine then re-metadata" },
];
  ...
  return (
    <>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        headline: "How to Edit a PDF's Title, Author, Keywords Online — Free",
        description: "Rewrite a PDF's title, author, subject, keywords, creator. Free, no sign-up, no watermark.",
        ...
        mainEntityOfPage: { "@type": "WebPage", "@id": "https://convertpdfgo.com/metadata" },
        speakable: { "@type": "SpeakableSpecification", cssSelector: ["h1", ".speakable-faq"] },
        inLanguage: "en", about: { "@type": "Thing", name: "Edit Metadata" },
        keywords: "edit pdf metadata, change pdf title, pdf author, pdf keywords",
      }) }} />

      <ToolPageShell tool={tool} locale={locale} index="21" variant="title · author · keywords">
        <MetadataClient />
        ...
        <div className="mt-8 sm:mt-12 grid gap-6 lg:gap-8 lg:grid-cols-[1fr_1fr]">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Edit Metadata FAQ</h2>
