...
const Body = (
  <>
    <Lead>Conferences hand out printed agendas that need a QR to the live session list. Restaurant menus need a QR to online ordering. Real estate listings need a QR to the virtual tour. Adding a QR to an existing PDF takes ten seconds and turns paper into a phone-tap.</Lead>

    <Section>
      <H2 id="why">When you actually need Add QR Code</H2>
      <p>Event programs linking to live sessions, brochures linking to booking pages, business cards as printed PDFs, signed contracts that need a verification link, real-estate listings linking to virtual tours.</p>
    </Section>

    <Section>
      <H2 id="how">How to add qr code, step by step</H2>
      <Step n={1} title="Open the tool">Go to <Link href="/qr-code" className="text-primary hover:underline">convertpdfgo.com/qr-code</Link>. No account, no email, no installation.</Step>
      ...
      <Step n={3} title="Configure your run">Enter the URL or text, set the corner and size.</Step>
    ...
    <Section>
      ...
      <Problem title="QR code is too small to scan">100 points (~1.4 inch) is the floor for desk reading. Go to 150-200 for posters.</Problem>
      <Problem title="QR scanned but link is broken">Test the URL in a browser before embedding. We don't validate — whatever text you type becomes the QR payload.</Problem>
      <Problem title="QR appears on wrong page">Page range selector — by default we add to all pages. Set "1" to add only to page 1.</Problem>
    </Section>
    ...
    <Section>
      ...
      <Faq q="What content types work?">Any text. URLs are most common, but vCards (contact info), Wi-Fi credentials, signed tokens, plain text all work.</Faq>
      <Faq q="How big should the QR code be?">100 points (~1.4 inch) is good for desk reading. Bigger if recipients scan from across a room.</Faq>
    ...
    <Section>
      ...
      <Callout>Now you can <Link href="/watermark" className="font-semibold text-primary hover:underline">Watermark</Link>, <Link href="/header-footer" className="font-semibold text-primary hover:underline">Header & Footer</Link>, or <Link href="/merge" className="font-semibold text-primary hover:underline">Merge PDF</Link>. All free.</Callout>
      <p>Or browse <Link href="/tools" className="text-primary hover:underline">our full list of 49 free PDF tools</Link>.</p>
    </Section>
  </>
);

export const qrCodePdfPost: BlogPost = {
  slug: "how-to-add-qr-code-to-pdf-online-free",
  title: "How to Add a QR Code to a PDF Online — Free, Any Content",
  description: "Embed a QR code on any page of a PDF — link, vCard, Wi-Fi credentials. Free, no sign-up, no watermark.",
  date: "2021-05-25",
  modified: "2026-06-01",
  author: "convertpdfgo team",
  tag: "Guide",
  readingMinutes: 7,
  keywords: ["add qr code to pdf", "pdf qr code", "qr code pdf online free", "embed qr in pdf"],
  heroEmoji: "📲",
  toc: [
