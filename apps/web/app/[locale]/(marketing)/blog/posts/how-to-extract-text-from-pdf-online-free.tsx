...
const Body = (
  <>
    <Lead>A 200-page government PDF holds a single statistic you need to quote. A scanned report has data your spreadsheet wants. Manual copy-paste page by page is a half-hour of clicking; extracting all text at once is two seconds.</Lead>

    <Section>
      <H2 id="why">When you actually need Extract Text</H2>
      <p>Quoting from government PDFs, feeding data to a spreadsheet, archiving full-text for search systems, scripting text analysis pipelines, prepping content for translation services.</p>
    </Section>

    <Section>
      <H2 id="how">How to extract text, step by step</H2>
      <Step n={1} title="Open the tool">Go to <Link href="/extract-text" className="text-primary hover:underline">convertpdfgo.com/extract-text</Link>. No account, no email, no installation.</Step>
      ...
      <Step n={3} title="Configure your run">Pick txt or json.</Step>
    ...
    <Section>
      ...
      <Problem title="Text comes out as ?????">The PDF uses a non-standard encoding or embedded subset fonts. Extract still works on most, but unusual scripts may need OCR as a fallback.</Problem>
      <Problem title="Tables come out as one column">Plain text loses spatial layout. Use PDF to Excel for tables that need to stay structured.</Problem>
      <Problem title="Scanned PDF returns empty">It's all images — there's no text layer to extract. OCR the file first, then Extract Text.</Problem>
    </Section>
    ...
    <Section>
      ...
      <Faq q="Will it work on scanned PDFs?">Only if the scan was OCR'd first. Pure image-scans have no extractable text. Run them through OCR before Extract Text.</Faq>
      <Faq q="What's the JSON format?">Per-page array: <code>{`[{"page": 1, "text": "..."}, ...]`}</code>. Easy to consume in scripts or data pipelines.</Faq>
    ...
    <Section>
      ...
      <Callout>Now you can <Link href="/pdf-to-word" className="font-semibold text-primary hover:underline">PDF to Word</Link>, <Link href="/ocr" className="font-semibold text-primary hover:underline">OCR PDF</Link>, or <Link href="/inspect" className="font-semibold text-primary hover:underline">Inspect PDF</Link>. All free.</Callout>
      <p>Or browse <Link href="/tools" className="text-primary hover:underline">our full list of 49 free PDF tools</Link>.</p>
    </Section>
  </>
);

export const extractTextPost: BlogPost = {
  slug: "how-to-extract-text-from-pdf-online-free",
  title: "How to Extract Text from a PDF Online — Free, TXT or JSON",
  description: "Get plain text out of any text-based PDF — copy-paste ready, or per-page JSON. Free, no sign-up, no watermark.",
  date: "2021-03-18",
  modified: "2026-06-01",
  author: "convertpdfgo team",
  tag: "Guide",
  readingMinutes: 7,
  keywords: ["extract text from pdf", "pdf to text", "pdf text extract", "copy text from pdf", "pdf to txt online free"],
  heroEmoji: "📝",
  toc: [
