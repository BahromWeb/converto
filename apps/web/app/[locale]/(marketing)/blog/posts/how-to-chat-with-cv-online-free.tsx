...
const Body = (
  <>
    <Lead>Sourcing a Go backend role and 80 CVs sit in the queue. Sifting them by hand is half a day — reading each one cover-to-cover, scoring loosely, missing the lateral-fit candidates because their resume doesn't say the magic words. Chat-with-CV turns each candidate into a 30-second screening conversation: skills match, gap check, salary expectations — all with citations.</Lead>

    <Section>
      <H2 id="why">When you actually need Chat with CV</H2>
      <p>Recruiters first-pass screening 50+ CVs, hiring managers comparing finalist candidates with the same prompt, founders triaging cold inbound resumes, candidates auditing their own resume against a target JD.</p>
    </Section>

    <Section>
      <H2 id="how">How to chat with cv, step by step</H2>
      <Step n={1} title="Open the tool">Go to <Link href="/chat-with-cv" className="text-primary hover:underline">convertpdfgo.com/chat-with-cv</Link>. No account, no email, no installation.</Step>
      <Step n={2} title="Drop the file in">Drag a CV (PDF or DOCX). Up to 30 MB. The file is encrypted in transit and at rest and auto-deleted within one hour — important for candidate confidentiality.</Step>
      <Step n={3} title="Wait a few seconds while we index">We chunk each section of the CV separately so skill claims, employment history, and education each get their own retrievable block.</Step>
    ...
    <Section>
      ...
      <Problem title="Says skill is present but actually isn't">If the resume mentions a tool only in a course-list (not in employment history), the AI sometimes counts it as production experience. Ask precisely: "Did this candidate use Kafka in a paid job, or just learn it on a course?" — the citation will tell you which.</Problem>
      <Problem title="Misses lateral-fit candidates">AI screens for keyword presence. Candidates with adjacent experience (Rust + Python who could ramp to Go in a month) get rejected if you ask "does this CV mention Go?". Ask "could this candidate learn Go quickly given their background?" to surface laterals.</Problem>
      <Problem title="Gap analysis flags false positives">Many CVs omit short gaps between roles. The AI flags every unexplained span &gt;3 months — including freelance periods labelled vaguely. Ask the candidate before scoring it as a red flag.</Problem>
    </Section>
    ...
    <Section>
      ...
      <Faq q="Will it match against my job description?">Indirectly. Paste the JD into your first message: "Compare this CV to the JD I'm pasting: [JD text]" and the AI scores fit by skill.</Faq>
      <Faq q="Can it spot embellishments?">It can flag inconsistencies in dates and ranges, but it can't verify external claims ("managed a team of 50") against truth.</Faq>
      <Faq q="Are my files private?">Files are encrypted in transit (TLS 1.3) and at rest, then auto-deleted within one hour. Candidate data is not used for training.</Faq>
    ...
    <Section>
      ...
      <Callout>Now you can <Link href="/chat" className="font-semibold text-primary hover:underline">Chat with PDF</Link>, <Link href="/chat-word" className="font-semibold text-primary hover:underline">Chat with Word</Link>, or <Link href="/extract-text" className="font-semibold text-primary hover:underline">Extract Text</Link>. All free.</Callout>
      <p>Or browse <Link href="/tools" className="text-primary hover:underline">our full list of 49 free PDF tools</Link>.</p>
    </Section>
  </>
);

export const chatCvPost: BlogPost = {
  slug: "how-to-chat-with-cv-online-free",
  title: "How to Chat with a CV Online — Free, Screen Resumes in 30 Seconds",
  description: "Drop any resume and ask screening questions in plain English — skills, gaps, years of experience. Free, no sign-up.",
  date: "2021-08-10",
  modified: "2026-06-01",
  author: "convertpdfgo team",
  tag: "Guide",
  readingMinutes: 8,
  keywords: ["chat with cv", "chat with resume", "resume ai", "screen cv ai", "candidate screening ai", "resume chat online free"],
  heroEmoji: "👔",
  toc: [
