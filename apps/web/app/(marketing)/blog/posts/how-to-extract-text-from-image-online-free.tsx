...
const Body = (
  <>
    <Lead>A screenshot has a paragraph you want to quote. A photograph of a sign in another country needs translating. The text is right there visually but you can't select it — it's pixels, not characters. OCR turns the pixels back into text.</Lead>

    <Section>
      <H2 id="why">When you actually need Image to Text</H2>
      <p>Quoting from screenshots, copy-pasting from photos of menus or signs, archiving user-interface text from old products, transcribing handwritten notes (poorly — Tesseract isn't great at handwriting).</p>
    </Section>

    <Section>
      <H2 id="how">How to image to text, step by step</H2>
      <Step n={1} title="Open the tool">Go to <Link href="/image-to-text" className="text-primary hover:underline">convertpdfgo.com/image-to-text</Link>. No account, no email, no installation.</Step>
      ...
      <Step n={3} title="Configure">Pick the OCR language. Returns plain text.</Step>
    ...
    <Section>
      ...
      <Problem title="Wrong language">Pass the right code: eng English, rus Russian, uzb Uzbek Latin, uzb_cyrl Cyrillic Uzbek, kir Kyrgyz.</Problem>
      <Problem title="Bad quality output on screenshot">Increase the screenshot resolution — Tesseract needs ~300 DPI equivalent. macOS Cmd+Shift+4 produces sharp PNGs.</Problem>
      <Problem title="Multi-column layout came out wrong">Tesseract reads top-to-bottom by default. For magazines/newspapers, crop columns separately then OCR each.</Problem>
    </Section>
    ...
    <Section>
      ...
      <Faq q="Will it work on a screenshot?">Yes — screenshots are just PNGs. Works especially well on UI text because the pixels are sharp and high-contrast.</Faq>
      <Faq q="Handwriting?">Tesseract is poor at handwriting. Stick to printed text. For handwriting, a dedicated handwriting OCR tool is needed.</Faq>
    ...
    <Section>
      ...
      <Callout>Now you can <Link href="/ocr-extract-text" className="font-semibold text-primary hover:underline">OCR Extract Text</Link>, <Link href="/ocr" className="font-semibold text-primary hover:underline">OCR PDF</Link>, or <Link href="/ocr-detect-lang" className="font-semibold text-primary hover:underline">Detect Language</Link>. All free.</Callout>
      <p>Or browse <Link href="/tools" className="text-primary hover:underline">our full list of 49 free PDF tools</Link>.</p>
    </Section>
  </>
);

export const imageToTextPost: BlogPost = {
  slug: "how-to-extract-text-from-image-online-free",
  title: "How to Extract Text from an Image or Screenshot — Free, OCR",
  description: "Read text from any JPG, PNG, or screenshot — Tesseract OCR in 100+ languages. Free, no sign-up.",
  date: "2020-12-28",
  modified: "2026-06-01",
  author: "convertpdfgo team",
  tag: "Guide",
  readingMinutes: 8,
  keywords: ["image to text", "extract text from image", "ocr image", "screenshot to text", "photo to text online free"],
  heroEmoji: "📷",
  toc: [
